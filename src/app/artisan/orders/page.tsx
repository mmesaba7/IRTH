        ...
        {error ? (
          ...
          <div className="mt-12 rounded-[var(--radius-lg)] bg-[var(--surface-muted)] p-10 text-center">
            ...
            <p className="mt-2 text-sm text-[var(--text-secondary)]">
              عندما يشتري عميل منتجًا تابعًا لك سيظهر طلبك هنا تلقائيًا.
            </p>
          </div>
        ) : (
            ...
            {orders.map((order) => (
              ...
              >
                <div className="grid gap-5 border-b border-[var(--border-soft)] p-6 md:grid-cols-[1.3fr_1fr_1fr]">
                  ...
                  <div>
                    <p className="text-xs font-medium uppercase tracking-[0.16em] text-[var(--text-muted)]">Customer</p>
                    <p className="mt-2 font-medium text-[var(--color-espresso)]">{order.customer_display_name ?? "Customer"}</p>
                    <p className="mt-1 text-sm text-[var(--text-secondary)]">{order.customer_country_code ?? "—"}</p>
                  </div>

                  <div>
                    <p className="text-xs font-medium uppercase tracking-[0.16em] text-[var(--text-muted)]">Your merchandise</p>
                    <p className="mt-2 text-xl font-medium text-[var(--color-copper)]">
                      {order.currency_code} {order.artisan_merchandise_subtotal}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid gap-4 border-b border-[var(--border-soft)] bg-[var(--surface-muted)]/50 px-6 py-4 sm:grid-cols-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-medium text-[var(--color-espresso)]">{statusLabel(order.order_status)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-[var(--text-muted)]">Your fulfillment status</p>
                    <p className="mt-1 text-sm font-medium text-[var(--color-espresso)]">{statusLabel(order.fulfillment_status)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-[var(--text-muted)]">Payment status</p>
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          ...
                          <div className="text-left sm:text-right">
                            <p className="text-sm text-[var(--text-secondary)]">
                              Unit: {order.currency_code} {item.unitPrice}
                            </p>
                            <p className="mt-1 font-medium text-[var(--color-copper)]">
                              Line: {order.currency_code} {item.lineTotal}
                            </p>
                          </div>
                        </div>
                        ...
                        {(hasPositiveMoney(item.promotionDiscount) || hasPositiveMoney(item.couponDiscount)) && (
                          <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 border-t border-[var(--border-soft)] pt-3 text-xs text-[var(--text-muted)]">
                            {hasPositiveMoney(item.promotionDiscount) && (
                              <span>Promotion discount: {order.currency_code} {item.promotionDiscount}</span>
                            )}
                            {hasPositiveMoney(item.couponDiscount) && (
                              <span>Coupon discount: {order.currency_code} {item.couponDiscount}</span>
                            )}
                          </div>
                        )}
                      </div>
