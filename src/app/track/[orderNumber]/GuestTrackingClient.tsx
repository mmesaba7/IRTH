...
export default function GuestTrackingClient({ orderNumber }: { orderNumber: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/order-tracking/guest", {
      ...
      .catch(() => {
        if (!controller.signal.aborted) setError("Unable to load tracking right now. Please try again.");
      })
      .finally(() => {
  ...
  if (loading) {
    return <div className="rounded-[var(--radius-lg)] border border-[var(--border-soft)] bg-[var(--surface)] p-8 text-center"><p className="text-sm text-[var(--text-secondary)]">Loading secure tracking…</p></div>;
  }

  if (!order) {
    return (
      <div className="rounded-[var(--radius-lg)] border border-[var(--border-soft)] bg-[var(--surface)] p-8 text-center">
        <h2 className="font-[var(--font-display)] text-2xl text-[var(--color-espresso)]">Tracking unavailable</h2>
        <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[var(--text-secondary)]">{error || "Tracking link is invalid or unavailable."}</p>
      </div>
    );
  }
