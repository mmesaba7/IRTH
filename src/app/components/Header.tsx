...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const classifyAccount = async (userId: string | null) => {
      if (!userId) { if (!cancelled) setAccountState("guest"); return; }
      const { data, error } = await supabase.from("user_accounts").select("user_id").eq("user_id", userId).maybeSingle();
      if (cancelled) return;
      setAccountState(!error && data ? "customer" : "authenticated");
    };
    supabase.auth.getUser().then(({ data }) => { void classifyAccount(data.user?.id ?? null); });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => { void classifyAccount(session?.user.id ?? null); });
  ...
  useEffect(() => {
    ...
    fetch("/api/cms/brand", { cache: "no-store" })
      .then(async (response) => { if (!response.ok) return null; const body = await response.json(); const value = body?.assets?.mainLogoAssetId; return typeof value === "string" ? value : null; })
      .then((url) => { if (!cancelled) setMainLogoUrl(url); })
      .catch(() => { if (!cancelled) setMainLogoUrl(null); });
  ...
  useEffect(() => {
    ...
    const closeAtDesktop = () => { if (desktopQuery.matches) setMenuOpen(false); };
    closeAtDesktop();
  ...
  const accountLink = accountState === "guest" ? { href: "/account/login", label: "Login" } : accountState === "customer" ? { href: "/account", label: "Account" } : null;
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[var(--color-petrol)] text-[var(--color-ivory)] shadow-[0_8px_28px_rgba(6,44,56,0.12)]">
      <div className="mx-auto max-w-[var(--container-max)] px-4 sm:px-5 md:px-6">
        <div className="flex min-h-[68px] items-center justify-between gap-4 md:min-h-[76px]">
          <div className="flex min-w-0 items-center gap-3 xl:gap-7">
            <button type="button" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)} className="irth-icon-button xl:hidden">
            ...
            <Link href="/" className="flex min-h-9 shrink-0 items-center text-[var(--color-ivory)]">
              {mainLogoUrl ? <img src={mainLogoUrl} alt="IRTH" className="h-9 w-auto max-w-[138px] object-contain" /> : <span className="font-[var(--font-display)] text-[1.65rem] font-semibold tracking-[0.13em]">IRTH</span>}
            </Link>
            <nav className="hidden items-center gap-4 text-[13px] font-medium xl:flex xl:gap-6">
              {primaryLinks.map((link) => <Link key={link.href} href={link.href} className="text-[var(--color-ivory)]/78 transition-colors hover:text-[var(--color-antique-gold)]">{link.label}</Link>)}
          ...
          <div className="flex shrink-0 items-center gap-1 sm:gap-2">
            <Link href="/search" aria-label="Search" className="irth-icon-button"><IrthIcon name="search" className="h-5 w-5" /></Link>
            <div className="hidden items-center gap-2 md:flex">
              <NotificationBell />
              <Link href="/saved" aria-label="Saved items" className="irth-icon-button relative"><IrthIcon name="heart" className="h-5 w-5" />{savedCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--color-copper)] px-1 text-[9px] text-white">{savedCount > 99 ? "99+" : savedCount}</span>}</Link>
            </div>
            <Link href="/cart" aria-label="Shopping cart" className="irth-icon-button relative"><IrthIcon name="cart" className="h-5 w-5" />{cartCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--color-copper)] px-1 text-[9px] text-white">{cartCount > 99 ? "99+" : cartCount}</span>}</Link>
            <div className="hidden items-center gap-3 xl:flex">
              <div className="border-s border-white/15 ps-3"><MarketSelector /></div>
              {isMounted && <button type="button" onClick={toggleLocale} className="text-xs font-semibold text-[var(--color-ivory)]/75 transition hover:text-[var(--color-antique-gold)]">{locale === "ar" ? "EN" : "عربي"}</button>}
              {accountLink && <Link href={accountLink.href} aria-label={accountLink.label} className="irth-icon-button"><IrthIcon name="user" className="h-5 w-5" /></Link>}
            </div>
          </div>
        </div>
      </div>
      ...
      {!menuOpen && (
        <div className="border-t border-white/10 bg-[var(--color-petrol-deep)] px-4 py-2 xl:hidden">
          <div className="mx-auto flex max-w-[var(--container-max)] items-center justify-between gap-3">
            <span className="shrink-0 text-[11px] font-semibold uppercase tracking-[.12em] text-[var(--color-antique-gold)]">Your market</span>
            <MarketSelector />
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 top-[68px] z-40 overflow-y-auto bg-[var(--color-petrol-deep)] text-[var(--color-ivory)] md:top-[76px] xl:hidden">
          <div className="mx-auto flex min-h-full max-w-xl flex-col px-6 pb-10 pt-8">
            <div className="grid gap-1">
              {primaryLinks.map((link) => <Link key={link.href} href={link.href} onClick={closeMenu} className="flex items-center justify-between border-b border-white/8 py-4 text-lg font-medium"><span>{link.label}</span><span className="text-[var(--color-antique-gold)]">›</span></Link>)}
            ...
            <div className="mt-8 rounded-[var(--radius-lg)] border border-white/10 bg-white/5 p-5">
              <div className="flex items-center justify-between gap-4"><span className="text-sm font-semibold">Notifications</span><NotificationBell /></div>
              <div className="mt-5 border-t border-white/10 pt-5"><p className="mb-3 text-xs uppercase tracking-[.16em] text-[var(--color-antique-gold)]">Market</p><MarketSelector /></div>
              {isMounted && <button type="button" onClick={toggleLocale} className="mt-5 flex w-full items-center justify-between border-t border-white/10 pt-5 text-sm"><span>Language</span><span className="font-semibold text-[var(--color-antique-gold)]">{locale === "ar" ? "English" : "العربية"}</span></button>}
            </div>
          </div>
        </div>
      )}
