    ...
    >
      <summary className="cursor-pointer list-none px-6 py-5 transition hover:bg-[var(--surface-muted)]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="font-mono text-sm font-semibold text-[var(--color-espresso)]">
              {order.orderNumber}
            </p>
            <p className="mt-1 text-xs text-[var(--text-muted)]">
              {formatDate(order.createdAt)} · {order.items.length} {order.items.length === 1 ? "product" : "products"}
            </p>
          </div>
          <div className="text-left sm:text-right">
            <p className="text-sm font-medium text-[var(--color-olive)]">
              {orderStatusLabel(order.status)}
            </p>
            <p className="mt-1 font-medium text-[var(--color-copper)]">
              {order.currencyCode} {order.finalTotal}
            </p>
          </div>
        </div>
      </summary>
        ...
        {exceptionalStatus && (
          <div className="mb-6 rounded-[var(--radius-md)] border border-[var(--border-soft)] bg-[var(--surface-muted)] p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">
              This order is in an exceptional state. IRTH support will handle the next operational step.
            </p>
          </div>
        )}
              ...
              return (
                <div key={status} className="flex gap-3">
                  ...
                  <div className="min-w-0 pb-2">
                    <p className={`text-sm font-medium ${reached ? "text-[var(--color-espresso)]" : "text-[var(--text-muted)]"}`}>
                      {orderStatusLabel(status)}
                    </p>
                    <p className="mt-0.5 text-xs text-[var(--text-muted)]">
                      {exactTimestamp ? formatDate(exactTimestamp) : reached ? "Completed" : "Pending"}
                    </p>
                  </div>
                </div>
              );
        ...
        <section className="mt-7 border-t border-[var(--border-soft)] pt-6">
          ...
          <div className="mt-4 space-y-3">
            {order.items.map((item) => (
              <div key={`${item.slug}-${item.quantity}`} className="flex items-start justify-between gap-4 text-sm">
                <div>
                ...
                <p className="shrink-0 text-[var(--color-copper)]">
        ...
        <section className="mt-7 border-t border-[var(--border-soft)] pt-6">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              ...
              <span>{order.currencyCode} {order.subtotalBeforePromotions}</span>
            </div>
            {isPositiveMoney(order.promotionDiscountTotal) && (
              <div className="flex justify-between gap-4 text-[var(--color-olive)]">
                <span>Promotions</span>
                <span>− {order.currencyCode} {order.promotionDiscountTotal}</span>
              </div>
            )}
            {isPositiveMoney(order.couponDiscountTotal) && (
              <div className="flex justify-between gap-4 text-[var(--color-olive)]">
                <span>Coupon{order.couponCode ? ` (${order.couponCode})` : ""}</span>
                <span>− {order.currencyCode} {order.couponDiscountTotal}</span>
              </div>
            )}
            <div className="flex justify-between gap-4">
              <span className="text-[var(--text-secondary)]">Merchandise subtotal</span>
              <span>{order.currencyCode} {order.merchandiseSubtotal}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-[var(--text-secondary)]">Shipping</span>
              <span>{order.currencyCode} {order.shippingFee}</span>
            </div>
            <div className="flex justify-between gap-4 border-t border-[var(--border-soft)] pt-3 font-medium text-[var(--color-espresso)]">
              <span>Final total</span>
              <span>{order.currencyCode} {order.finalTotal}</span>
            </div>
            <div className="flex justify-between gap-4 pt-1">
              <span className="text-[var(--text-secondary)]">Payment status</span>
              <span>{orderStatusLabel(order.paymentStatus)}</span>
            </div>
          </div>
        </section>
        ...
        <section className="mt-7 border-t border-[var(--border-soft)] pt-6">
          ...
          {order.shipments.length === 0 ? (
            <p className="mt-4 text-sm text-[var(--text-secondary)]">
              Tracking will appear here once a shipment is created.
            </p>
          ) : (
            <div className="mt-4 space-y-4">
              {order.shipments.map((shipment, index) => {
                ...
                return (
                  <div
                    key={`${shipment.createdAt}-${index}`}
                    className="rounded-[var(--radius-md)] border border-[var(--border-soft)] bg-[var(--surface-muted)] p-4"
                  >
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      {shipment.courierCode && (
                        <p className="text-xs uppercase tracking-[0.12em] text-[var(--text-muted)]">
                          {shipment.courierCode.replaceAll("_", " ")}
                        </p>
                      )}
                    </div>

                    {shipment.trackingNumber && (
                      <p className="mt-3 break-all font-mono text-sm text-[var(--color-espresso)]">
                        {shipment.trackingNumber}
                      </p>
                    )}
                    ...
                    {trackingUrl && (
                      <Link
                        ...
                        className="mt-3 inline-block text-sm font-medium text-[var(--color-copper)] hover:underline"
                      >
