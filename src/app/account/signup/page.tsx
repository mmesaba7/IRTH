      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl items-center justify-center sm:min-h-[calc(100vh-6rem)]">
        <div className="grid w-full overflow-hidden rounded-[var(--radius-xl)] bg-[var(--surface)] shadow-[var(--shadow-elevated)] lg:grid-cols-[0.85fr_1.15fr]">
          <aside className="relative hidden overflow-hidden bg-[var(--color-espresso)] p-10 text-[var(--color-ivory)] lg:flex lg:flex-col lg:justify-between">
            ...
            <Link href="/" className="relative z-10 font-[var(--font-display)] text-3xl tracking-[0.12em]">IRTH</Link>
            <div className="relative z-10 max-w-xs">
              <p className="text-xs font-medium uppercase tracking-[0.22em] text-[var(--color-copper)]">Join IRTH</p>
              <p className="mt-4 font-[var(--font-display)] text-4xl leading-tight">Keep the crafts and stories you care about close.</p>
            </div>
          </aside>
          ...
          <section className="px-6 py-9 sm:px-10 sm:py-12 lg:px-14 lg:py-16">
            ...
            <h1 className="mt-3 font-[var(--font-display)] text-4xl text-[var(--color-espresso)] sm:text-5xl">Create IRTH Account</h1>
            <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">Create an optional customer account to keep your orders and shopping activity together.</p>

            <form action={customerSignup} className="mt-9 space-y-5">
              ...
              {params.error && (
                <p aria-live="polite" className="rounded-[var(--radius-md)] border border-red-200 bg-red-50 p-3 text-sm text-red-700">لم نتمكن من إنشاء الحساب.</p>
              )}
            ...
            <p className="mt-7 border-t border-[var(--border-soft)] pt-6 text-sm text-[var(--text-secondary)]">
              ...
              <Link href="/account/login" className="font-medium text-[var(--color-copper)] hover:underline">Login</Link>
            </p>
          </section>
        </div>
      </div>
